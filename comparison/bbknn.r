...
parser$add_argument("--method", type = "character", default = "bbknn")
...
# convert Seurat data to AnnaData
tmp_dir <- pj(output_dir, "tmp")
mkdir(tmp_dir, remove_old = T)

DefaultAssay(obj) <- "atac"
fname <- pj(tmp_dir, "obj_atac.h5Seurat")
SaveH5Seurat(obj, filename = fname, overwrite = T)
Convert(fname, dest = "h5ad", overwrite = T)
adata_atac <- sc$read_h5ad(pj(tmp_dir, "obj_atac.h5ad"))

DefaultAssay(obj) <- "rna"
fname <- pj(tmp_dir, "obj_rna.h5Seurat")
SaveH5Seurat(obj, filename = fname, overwrite = T)
Convert(fname, dest = "h5ad", overwrite = T)
adata_rna <- sc$read_h5ad(pj(tmp_dir, "obj_rna.h5ad"))

DefaultAssay(obj) <- "adt"
fname <- pj(tmp_dir, "obj_adt.h5Seurat")
SaveH5Seurat(obj, filename = fname, overwrite = T)
Convert(fname, dest = "h5ad", overwrite = T)
adata_adt <- sc$read_h5ad(pj(tmp_dir, "obj_adt.h5ad"))

unlink(tmp_dir, recursive = T)

# run bbknn
sc$external$pp$bbknn(adata_atac, batch_key = "batch", use_rep = "X_lsi")
sc$external$pp$bbknn(adata_rna, batch_key = "batch", use_rep = "X_pca_rna")
sc$external$pp$bbknn(adata_adt, batch_key = "batch", use_rep = "X_pca_adt")

# average connectivities
connectivities <- (adata_rna$obsp["connectivities"] + adata_adt$obsp["connectivities"] +
                   adata_atac$obsp["connectivities"]) / 3
rownames(connectivities) <- colnames(connectivities) <- colnames(obj)
invisible(writeMM(connectivities, pj(output_dir, "connectivities.mtx")))

n_neighbor <- 19
connectivities_neigh <- connectivities
order_matrix <- t(apply(connectivities, 1, order, decreasing = T))
idx <- order_matrix[, 1:n_neighbor]
seq_len_conn <- seq_len(nrow(connectivities))
for (i in seq_len_conn) {
    connectivities_neigh[i, ][setdiff(seq_len_conn, idx[i, ])] <- 0
}
rownames(connectivities_neigh) <- colnames(connectivities_neigh) <- colnames(obj)
obj[["weighted.nn"]] <- as.Neighbor(as.Graph(connectivities_neigh))

...
parser$add_argument("--method", type = "character", default = "seurat_rpca+wnn")
...
obj <- RunPCA(obj, reduction.name = "pca_rpca_rna")
...
obj <- ScaleData(obj, verbose = FALSE)
obj <- RunPCA(obj, reduction.name = "pca_rpca_adt")
...
obj <- RunSVD(obj, reduction.name = "pca_rpca_atac")

# wnn
obj <- FindMultiModalNeighbors(obj, list("pca_rpca_atac", "pca_rpca_rna", "pca_rpca_adt"),
                                    list(1:K, 1:K, 1:K))

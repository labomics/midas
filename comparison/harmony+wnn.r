...
parser$add_argument("--method", type = "character", default = "harmony+wnn")
...
# harmony
obj <-  RunHarmony(obj, "batch", reduction = "lsi", assay.use = "atac", project.dim = F,
            reduction.save = "harmony_atac") %>%
        RunHarmony("batch", reduction = "pca_rna",  assay.use = "rna", project.dim = F,
            reduction.save = "harmony_rna") %>%
        RunHarmony("batch", reduction = "pca_adt",  assay.use = "adt", project.dim = F,
            reduction.save = "harmony_adt")

# wnn
obj <- FindMultiModalNeighbors(obj, list("harmony_atac", "harmony_rna", "harmony_adt"),
                                    list(1:K, 1:K, 1:K))

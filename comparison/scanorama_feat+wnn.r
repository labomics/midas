...
parser$add_argument("--method", type = "character", default = "scanorama_feat+wnn")
...
rm(obj, rna_list, atac_list, adt_list, cell_name_list, rna_subset_list, adt_subset_list, atac_subset_list)
gc()
...
# rna
corrected.rna <- scanorama$correct(rna_assay_list, rna_gene_list, return_dimred=FALSE, return_dense=TRUE)
int_rna_data <- lapply(corrected.rna[[1]], t)
panorama_rna <- do.call(cbind, int_rna_data)
rownames(panorama_rna) <- as.character(corrected.rna[[2]])
colnames(panorama_rna) <- unlist(sapply(rna_assay_list, rownames))
obj <- CreateSeuratObject(counts = panorama_rna, assay = "rna_bc")
rm(corrected.rna, int_rna_data, panorama_rna)
gc()

# adt
corrected.adt <- scanorama$correct(adt_assay_list, adt_gene_list, return_dimred=FALSE, return_dense=TRUE)
int_adt_data <- lapply(corrected.adt[[1]], t)
panorama_adt <- do.call(cbind, int_adt_data)
rownames(panorama_adt) <- as.character(corrected.adt[[2]])
colnames(panorama_adt) <- unlist(sapply(adt_assay_list, rownames))
obj[["adt_bc"]] <- CreateAssayObject(counts = panorama_adt)
rm(corrected.adt, int_adt_data, panorama_adt)
gc()

# atac
corrected.atac <- scanorama$correct(atac_assay_list, atac_gene_list, return_dimred=FALSE, return_dense=TRUE)
int_atac_data <- lapply(corrected.atac[[1]], t)
panorama_atac <- do.call(cbind, int_atac_data)
rownames(panorama_atac) <- as.character(corrected.atac[[2]])
colnames(panorama_atac) <- unlist(sapply(atac_assay_list, rownames))
obj[["atac_bc"]] <- CreateChromatinAssay(counts = panorama_atac)
rm(corrected.atac, int_atac_data, panorama_atac)
gc()

obj@meta.data$l1 <- do.call("c", unname(label_list))
obj@meta.data$batch <- factor(x = do.call("c", unname(subset_name_list)), levels = subset_names)

obj <- dim_reduc(obj, atac = "atac_bc", rna = "rna_bc", adt = "adt_bc")
...
# obj <- LoadH5Seurat(pj(output_dir, "obj.h5seurat"), assays = "adt_bc", reductions = "umap")
